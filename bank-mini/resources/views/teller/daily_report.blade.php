            ...
            <div class="bg-white rounded-lg border border-slate-200 shadow-sm p-6">
                <div class="flex items-center justify-between border-b border-slate-100 pb-4 mb-4">
                    <div>
                        ...
                        <span class="text-xs text-slate-500 font-mono">Tanggal: {{ date('d F Y', strtotime($reportData['report_date'])) }}</span>
                    </div>
                    @if($reportData['existing_report'])
                        ...
                        @endif
                    @else
                        <span class="px-2.5 py-1 bg-slate-100 text-slate-600 border border-slate-200 font-semibold text-xs rounded">
                            Belum Disubmit
                        </span>
                    @endif
                </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
                    <div class="p-4 bg-slate-50 rounded-md border border-slate-200">
                        <span class="text-xs font-semibold text-slate-500 uppercase tracking-wider block">1. Saldo Kas Awal</span>
                        <span class="text-xl font-bold font-mono text-slate-800 block mt-1">Rp {{ number_format($reportData['opening_cash'], 0, ',', '.') }}</span>
                    </div>
                    <div class="p-4 bg-blue-50/60 rounded-md border border-blue-200">
                        <span class="text-xs font-semibold text-blue-800 uppercase tracking-wider block">2. Total Setoran (+)</span>
                        <span class="text-xl font-bold font-mono text-blue-700 block mt-1">Rp {{ number_format($reportData['total_deposit'], 0, ',', '.') }}</span>
                    </div>
                    <div class="p-4 bg-slate-50 rounded-md border border-slate-200">
                        <span class="text-xs font-semibold text-slate-700 uppercase tracking-wider block">3. Total Penarikan (-)</span>
                        <span class="text-xl font-bold font-mono text-slate-800 block mt-1">Rp {{ number_format($reportData['total_withdrawal'], 0, ',', '.') }}</span>
                    </div>
                    <div class="p-4 bg-blue-50 rounded-md shadow-sm border border-blue-200">
                        <span class="text-xs font-semibold text-blue-900 uppercase tracking-wider block">4. Saldo Kas Sistem</span>
                        <span class="text-xl font-bold font-mono text-blue-700 block mt-1" id="system_balance" data-value="{{ $reportData['expected_closing_cash'] }}">
                ...
                <form action="{{ route('teller.daily-report.store') }}" method="POST" class="space-y-4 pt-4 border-t border-slate-100">
                    @csrf

                    <div>
                        <label for="physical_cash" class="block text-xs font-semibold uppercase tracking-wider text-slate-700 mb-1.5">
                    ...
                    <div id="match_status_box" class="p-3.5 rounded-md text-xs font-medium transition flex items-center gap-2">
                        <!-- JS injected status -->
                    </div>

                    <div class="flex items-center justify-end gap-3 pt-2">
                        <button type="submit" id="submit_btn" class="px-5 py-2 bg-blue-600 hover:bg-blue-700 text-white font-medium text-xs rounded-md shadow-sm transition flex items-center gap-1.5">
